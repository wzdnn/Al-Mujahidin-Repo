...
if (empty($_SESSION['admin'])) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if ($_REQUEST['kode'] == "" || $_REQUEST['nama'] == "" || $_REQUEST['tujuan'] == "" || $_REQUEST['penerima'] == "") {
            ...
            if (!preg_match("/^[a-zA-Z0-9. ]*$/", $kode)) {
                ...
                if (!preg_match("/^[a-zA-Z0-9.,\/ -]*$/", $nama)) {
                    ...
                    if (!preg_match("/^[a-zA-Z0-9.,()\/\r\n -]*$/", $tujuan)) {
                        $_SESSION['tujuan'] = 'Form Tujuan hanya boleh mengandung  huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan kurung()';
                        echo '<script language="javascript">window.history.back();</script>';
                    } else {

                        if (!preg_match("/^[a-zA-Z0-9.,\/ -]*$/", $penerima)) {
                            $_SESSION['penerima'] = 'Form Nama hanya boleh mengandung karakter huruf, spasi, titik(.), koma(,) dan minus(-)';
                            echo '<script language="javascript">window.history.back();</script>';
                        } else {

                            $query = mysqli_query($config, "UPDATE tbl_klasifikasi SET kode='$kode', nama='$nama', tujuan='$tujuan', id_user='$id_user', penerima='$penerima' WHERE id_klasifikasi='$id_klasifikasi'");
                    ...
                    <form class="col s12" method="post" action="?page=ref&act=edit">
                        ...
                        <div class="row">
                            ...
                            <div class="input-field col s12">
                                ...
                                <textarea id="penerima" class="materialize-textarea" name="penerima" required><?php echo $row['penerima']; ?></textarea>
                                <?php
                                if (isset($_SESSION['penerima'])) {
                                    $tujuan = $_SESSION['penerima'];
                                    echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $penerima . '</div>';
                                    unset($_SESSION['penerima']);
                                }
                                ?>
                            ...
                            <div class="input-field col s12">
                                ...
                                <textarea id="tujuan" class="materialize-textarea" name="tujuan" required><?php echo $row['tujuan']; ?></textarea>
                                <?php
